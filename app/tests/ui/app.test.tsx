...
describe("shared application", () => {
  ...
  it("identifies a score generated by the current pipeline revision", async () => {
    renderApp();
  ...
  it("explains that legacy cursor metadata needs a score refresh", async () => {
    const fixture = createFixtureRuntime();
    const runtime = new Proxy(fixture, {
      get(target, property) {
        if (property === "listScoreVariants") {
          return async (
            ...args: Parameters<AtpianoRuntime["listScoreVariants"]>
          ) => {
            const page = await target.listScoreVariants(...args);
            return {
              ...page,
              producer: null,
              freshness: {
    ...
    expect(
      await screen.findByText(
        "This score uses legacy cursor metadata. Refresh score to generate the current playback mapping.",
      ),
    ).toBeTruthy();
    expect(
      screen.getByText("Score revision · untracked · incompatible"),
    ).toBeTruthy();
    expect(
      screen.queryByText(
