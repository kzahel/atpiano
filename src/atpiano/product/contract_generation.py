...
OPENAPI_RELATIVE_PATH = Path("contracts/openapi/atpiano-product-v1.json")
TYPESCRIPT_RELATIVE_PATH = Path("product/src/generated/schema.ts")
...
def build_openapi_document() -> dict[str, Any]:
    _, combined = models_json_schema(
        [(model, "validation") for model in product_models()],
    ...
    return {
        "openapi": "3.1.0",
        "info": {
            "title": "atpiano product contract",
            "version": "1.0.0",
            "description": (
                "Platform-neutral product operations. Musical time is always "
...
def _generate_typescript(
    *,
    ...
    executable = root / "product" / "node_modules" / ".bin" / "openapi-typescript"
    if not executable.is_file():
        raise RuntimeError(
            "TypeScript dependencies are missing; run `npm ci --prefix product`"
        )
    subprocess.run(
    ...
    with tempfile.TemporaryDirectory(prefix="atpiano-contracts-") as temporary:
        ...
        if check:
            ...
            if drifted:
                ...
                raise RuntimeError(f"generated product contracts have drifted: {names}")
        else:
