...
class AtpianoCorrectedCaptureProcessor extends AudioWorkletProcessor {
  constructor() {
    super();
    ...
    this.port.onmessage = (event) => {
      ...
      this.flush();
      this.port.postMessage({ type: "stopped", frameCount: this.emittedFrames });
    };
  }
  ...
  process(inputs) {
    ...
    while (sourceOffset < channel.length) {
      ...
      this.pending.set(
        channel.subarray(sourceOffset, sourceOffset + accepted),
        this.pendingFrames
      );
...
registerProcessor("atpiano-corrected-capture", AtpianoCorrectedCaptureProcessor);
